...
constexpr bool ECU_MANAGER_ENABLED = true;  // Honda PGM-FI KWP2000 aktif
...
constexpr uint32_t ECU_EXTERNAL_TIMEOUT_MS = 3000; // Honda K-Line bisa lambat saat cold start
...
// Honda PGM-FI KWP2000 addresses
// ECU addr 0x10 (Honda standard), Tester 0xF1, Header 0x80 (ISO 14230 length-in-header)
constexpr uint8_t ECU_ADDR_DEFAULT   = 0x10;
constexpr uint8_t TESTER_ADDR_DEFAULT = 0xF1;
constexpr uint8_t KLINE_HEADER_DEFAULT = 0x80; // ISO 14230-4 KWP2000
...
// =====================================================================
// Honda PGM-FI KWP2000 Fast Init helper
// ISO 14230-2: 25ms LOW, 25ms HIGH, then send 0xC1 0x10 0xF1 0x81 CS
// =====================================================================
namespace HondaKwp {

// Kirim bit-bang fast init ke K-Line TX pin, lalu start UART
static void fastInit(uint8_t txPin, HardwareSerial& serial,
                     uint8_t rxPin, uint32_t baud) {
  // Pastikan UART nonaktif dulu selama bit-bang
  serial.end();
  pinMode(txPin, OUTPUT);

  // ISO 14230-2 fast init: 25 ms LOW, 25 ms HIGH
  ...
  delay(2); // biarkan line stabil

  // Start Communication request: C1 10 F1 81 [CS]
  // Header=0xC1 (format), Target=0x10 (Honda ECU), Source=0xF1 (tester)
  // Service=0x81 (StartCommunication)
  const uint8_t startComm[5] = {0xC1, 0x10, 0xF1, 0x81, 0x44};
  serial.write(startComm, sizeof(startComm));
  serial.flush();

  if (cfg::DEBUG_MODE) {
    Serial.println("[HONDA] Fast init sent: C1 10 F1 81 44");
  }
}

// Bangun frame KWP2000 ISO 14230 format 0x80:
// 80 TGT SRC LEN [DATA...] CS
static size_t buildFrame(uint8_t* out, size_t outSize,
                         uint8_t target, uint8_t source,
                         const uint8_t* data, uint8_t dataLen) {
  size_t total = 4u + dataLen + 1u;
  ...
  uint16_t cs = 0;
  for (size_t i = 0; i < 4u + dataLen; ++i) cs += out[i];
  out[4 + dataLen] = static_cast<uint8_t>(cs & 0xFF);
  return total;
}

// Bangun OBD Mode 01 request (satu PID)
static size_t buildOBDRequest(uint8_t* out, size_t outSize,
                               uint8_t target, uint8_t source,
                               uint8_t pid) {
  const uint8_t data[2] = {0x01, pid};
  return buildFrame(out, outSize, target, source, data, 2);
}

} // namespace HondaKwp

// =====================================================================
// KLineManager — Honda PGM-FI ISO 14230 KWP2000 Fast Init
// =====================================================================
class KLineManager {
public:
  KLineManager()
  ...
  bool begin(uint8_t rxPin = cfg::PIN_KLINE_RX,
    ...
    if (cfg::DEBUG_MODE) {
      ...
      Serial.println("[KLINE] Honda PGM-FI ISO 14230 KWP2000 mode");
      Serial.println("[KLINE] Transceiver MC33290/L9637D wajib digunakan!");
    }
    return true;
  }
  ...
  void update() {
    ...
    // Timeout: ECU tidak reply dalam 2 detik → reconnect
    if (_connected && ((now - _lastRxMs) > 2000UL)) {
      if (cfg::DEBUG_MODE) Serial.println("[KLINE] ECU timeout, reconnecting...");
      _connected = false;
      _initDone  = false;
      _state     = KLineState::Reconnecting;
      _errorCount++;
    ...
    // Belum konek → coba init setiap KLINE_RETRY_MS
    ...
    // Keep-alive: kirim TesterPresent tiap 2 detik agar ECU tidak disconnect
    if (_connected && ((now - _lastKeepAliveMs) > 2000UL)) {
      _lastKeepAliveMs = now;
      sendTesterPresent();
    }
  }

  // Kirim request OBD Mode 01 satu PID
  bool requestPid(uint8_t service, uint8_t pid, bool force = false) {
    if (!_enabled) return false;
    if (!_connected && !force) return false;

    uint8_t frame[16] = {};
    size_t frameLen = HondaKwp::buildOBDRequest(frame, sizeof(frame),
                                                 _ecuAddress, _testerAddress,
                                                 pid);
    if (frameLen == 0) return false;

    // Flush echo (MC33290 full-duplex: ESP32 akan terima echo byte-nya sendiri)
    flushEcho(frame, frameLen);

    _state = KLineState::WaitingResponse;
    if (cfg::DEBUG_MODE) {
      Serial.printf("[KLINE] REQ svc=0x%02X pid=0x%02X\n", service, pid);
    }
    return true;
  }
...
private:
  ...
  uint8_t  _rxBuffer[MAX_ECU_FRAME * 2]; // buffer lebih besar untuk echo
  size_t   _rxLen;
  ECUFrame _pendingFrame;
  bool     _haveFrame;

  // Honda ISO 14230 Fast Init
  void doFastInit() {
    if (cfg::DEBUG_MODE) Serial.println("[KLINE] Honda fast init...");

    // Flush buffer lama
    while (_serial.available()) _serial.read();
    _rxLen = 0;

    // Lakukan fast init bit-bang
    HondaKwp::fastInit(_txPin, _serial, _rxPin, _baud);

    // Tunggu respons StartCommunication (0xC1 / KeyBytes)
    // Honda biasanya reply 0x83 F1 10 C1 EA 8F [CS] atau serupa
    // Tunggu max 500 ms
    uint32_t t0 = millis();
    while ((millis() - t0) < 500UL) {
      if (_serial.available()) {
        uint8_t b = static_cast<uint8_t>(_serial.read());
        if (cfg::DEBUG_MODE) Serial.printf("[KLINE] INIT RX: 0x%02X\n", b);
        // Terima byte apapun dari ECU sebagai tanda ECU hidup
        _lastRxMs = millis();
        _connected = true;
        _initDone  = true;
        _protocol  = ProtocolType::KWP2000;
        _state     = KLineState::Connected;
        _lastKeepAliveMs = millis();
        if (cfg::DEBUG_MODE) Serial.println("[KLINE] Honda ECU OK - KWP2000 connected!");
        // Flush sisa bytes init
        delay(50);
        while (_serial.available()) _serial.read();
        _rxLen = 0;
        return;
    ...
    if (cfg::DEBUG_MODE) Serial.println("[KLINE] Init timeout, will retry...");
    _state = KLineState::Error;
  }

  // TesterPresent: 80 10 F1 01 3E [CS]
  void sendTesterPresent() {
    const uint8_t data[1] = {0x3E};
    uint8_t frame[8] = {};
    size_t len = HondaKwp::buildFrame(frame, sizeof(frame),
                                      _ecuAddress, _testerAddress,
                                      data, 1);
    if (len == 0) return;
    flushEcho(frame, len);
    if (cfg::DEBUG_MODE) Serial.println("[KLINE] TesterPresent sent");
  }

  // Kirim frame dan buang echo (MC33290 half-duplex loopback)
  void flushEcho(const uint8_t* frame, size_t len) {
    _serial.write(frame, len);
    _serial.flush();
    // Tunggu dan buang echo byte sejumlah len
    uint32_t t0 = millis();
    size_t got = 0;
    while (got < len && (millis() - t0) < 50UL) {
      if (_serial.available()) {
        _serial.read();
        got++;
      }
  ...
  void readIncoming() {
    ...
    // Frame minimum: header(1) + target(1) + source(1) + len(1) + data(>=1) + cs(1) = 6
    if (_rxLen < 6) return;

    // Honda 0x80 format: byte[0]=0x80 atau 0x83 atau 0x8x
    // Cari byte pertama yang valid (buang junk/echo)
    size_t start = 0;
    while (start < _rxLen && (_rxBuffer[start] & 0xC0) != 0x80) {
      start++;
    }
    if (start > 0) {
      memmove(_rxBuffer, _rxBuffer + start, _rxLen - start);
      _rxLen -= start;
    }
    if (_rxLen < 6) return;

    // Panjang data ada di byte [3] untuk format 0x80
    uint8_t dataLen = _rxBuffer[3];
    size_t frameLen = 4u + static_cast<size_t>(dataLen) + 1u;
    ...
    // Verifikasi checksum
    uint16_t cs = 0;
    for (size_t i = 0; i < 4u + dataLen; ++i) cs += _rxBuffer[i];
    if ((cs & 0xFF) != _rxBuffer[4 + dataLen]) {
      if (cfg::DEBUG_MODE) {
        Serial.printf("[KLINE] CS error: calc=0x%02X got=0x%02X\n",
                      static_cast<unsigned>(cs & 0xFF),
                      static_cast<unsigned>(_rxBuffer[4 + dataLen]));
      }
      // Buang satu byte dan coba lagi
    ...
    // Frame valid - decode ke ECUFrame
    ECUFrame frame;
    frame.header     = _rxBuffer[0];
    frame.target     = _rxBuffer[1];
    frame.source     = _rxBuffer[2];
    frame.length     = dataLen;
    frame.payloadLen = dataLen;
    frame.checksum   = _rxBuffer[4 + dataLen];
    frame.timestampMs = millis();
    frame.valid      = true;
    for (uint8_t i = 0; i < dataLen; ++i) {
      frame.payload[i] = _rxBuffer[4 + i];
    }
    ...
    if (!_connected) {
      ...
      _protocol  = ProtocolType::KWP2000;
      _state     = KLineState::Connected;
      if (cfg::DEBUG_MODE) Serial.println("[KLINE] ECU connected via data frame");
    }

    if (cfg::DEBUG_MODE) {
      Serial.printf("[KLINE] RX frame dataLen=%u svc=0x%02X pid=0x%02X\n",
                    static_cast<unsigned>(dataLen),
                    static_cast<unsigned>(dataLen > 0 ? frame.payload[0] : 0),
                    static_cast<unsigned>(dataLen > 1 ? frame.payload[1] : 0));
    }
...
public:
  void begin(bool enabled) {
    ...
    // Honda PGM-FI: request satu per satu dengan jeda minimal 50ms
    // Jangan spam terlalu cepat — ECU Honda sensitif terhadap flooding
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_ENGINE_RPM,    80);  // RPM ~12Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_SPEED,         80);  // Speed ~12Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_THROTTLE_POS, 100);  // TPS 10Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_MAP,           150); // MAP 6Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_O2_SENSOR_1,  200); // O2/AFR 5Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_COOLANT_TEMP, 500); // Suhu 2Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_IAT,           500); // IAT 2Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_BATTERY_VOLT, 1000); // Batt 1Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_STFT,          500); // STFT 2Hz
    addSlot(cfg::OBD_MODE_CURRENT, cfg::PID_LTFT,         1000); // LTFT 1Hz
  }
  ...
  void update(KLineManager& kline, SensorHub& hub) {
    ...
    if ((now - _lastProcessMs) < 50) return; // min 50ms antar request Honda
    _lastProcessMs = now;
...
private:
  ...
  void drawTopZone(const DashboardSnapshot& s) {
    ...
    drawPanel(_topSprite, 0, 0, cfg::TFT_WIDTH, cfg::ZONE_TOP_H, "HONDA PGM-FI", cfg::COL_ACCENT);

    // Mode & ECU status
    const char* mode = "LOCAL";
    uint16_t modeColor = cfg::COL_GRAY;
    if (s.simulationMode) {
      mode = "SIM";
      modeColor = cfg::COL_YELLOW;
    } else if (s.ecuEnabled && s.ecuOnline) {
      mode = "ECU OK";
      modeColor = cfg::COL_GREEN;
    } else if (s.ecuEnabled && !s.ecuOnline) {
      mode = "ECU...";
      modeColor = cfg::COL_ORANGE;
    }

    _topSprite.setTextDatum(TL_DATUM);
    _topSprite.setTextColor(cfg::COL_WHITE, cfg::COL_PANEL);
    _topSprite.drawString("K-LINE:", 310, 6, 2);
    _topSprite.setTextColor(modeColor, cfg::COL_PANEL);
    _topSprite.drawString(mode, 376, 6, 2);
